...
- 현재 서비스 구조가 단일 FastAPI 백엔드 중심이다.
- 운영 서버 배포와 백업 부담이 낮다.
- 세트상품/매핑/화면 기준 데이터 규모는 작다.
- Python 표준 `sqlite3` 또는 SQLAlchemy를 바로 사용할 수 있다.
- WAL 모드와 주기 백업을 적용하면 단일 운영 서비스에는 실용적이다.

단, 다음 조건이 생기면 PostgreSQL 전환을 검토한다.

- 동시 편집자가 여러 명이다.
- 사용자 계정/권한/감사 로그가 본격화된다.
- Cafe24 주문 스냅샷이 장기간 대량 보관된다.
- 배치 작업과 웹 요청이 동시에 DB를 강하게 사용한다.

권장 구현 방식:

- 1차: SQLite + SQLAlchemy + Alembic
- DB 파일 위치: 운영 서버의 앱 데이터 디렉터리. 예: `data/catchup.db`
- WAL 활성화
- 마이그레이션 락 적용
- 매 배포 전 DB, WAL, SHM 백업
...
구현 중 추가 설계가 필요한 영역은 사용자 계정/권한, PostgreSQL 전환, 장기 아카이브뿐이며 1차 DB 도입의 blocker는 아니다.
