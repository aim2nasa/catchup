...
- LU셀(정의됨) → 화면에서는 `매핑셀`로 표현한다.
- LU셀 없음 → 화면에서는 `미지정` 또는 공란으로 표현한다.
- 예외(고정 제외 등) → 별도 예외 표시(예: 보라색)

용어 원칙:

- `LU셀`은 개발/기획/계약 문서의 내부 용어로 유지한다.
- 사용자 화면, 확인 모달, 도움말 문구에서는 `LU셀` 대신 `매핑셀`을 사용한다.
- `매핑셀`의 사용자-facing 의미는 “U상품 판매를 L상품 집계에 반영하는 교차셀”이다.
- 사용자가 작업 영향을 이해해야 하는 곳에서는 `매핑셀`만 단독으로 쓰지 말고, “총판매/매출/수식에 반영”이라는 설명을 함께 제공한다.
