...
test.describe('상품코드 페이지', () => {
  ...
  test('계산식 셀은 기본 원문 수식만 보이고 자세히/더블클릭 때 계산 내역을 펼친다', async ({ page }) => {
    ...
    await expect(directRevenueTerm.locator('.formula-entity-card--left')).toContainText('P00000HT')
    const directProductLabel = directRevenueTerm.locator('.formula-entity-label').filter({ hasText: '상품' })
    const directOptionLabel = directRevenueTerm.locator('.formula-entity-label').filter({ hasText: '옵션' })
    await expect(directProductLabel).toHaveCount(1)
    await expect(directOptionLabel).toHaveCount(1)
    await expect(directProductLabel).toHaveCSS('background-color', 'rgb(219, 234, 254)')
    await expect(directOptionLabel).toHaveCSS('background-color', 'rgb(237, 233, 254)')
    await expect(directProductLabel).not.toHaveCSS('background-color', 'rgb(237, 233, 254)')
    await expect(directRevenueTerm.locator('.formula-entity-field--product .formula-entity-value')).toHaveAttribute('title', /라이코젯아이브로우/)
    await expect(directRevenueTerm.locator('.formula-entity-field--option .formula-entity-value')).toHaveAttribute('title', /A · -/)
    ...
    await expect(setRevenueTerm.locator('.formula-entity-role')).toContainText(['세트상품', '왼쪽상품'])
    ...
    await expect(setRevenueTerm.locator('.formula-entity-card--left')).toContainText('P00000HT')
    await expect(setRevenueTerm.locator('.formula-entity-card--left')).toContainText('A ·')
    await expect(setRevenueTerm.locator('.formula-entity-label').filter({ hasText: '상품' })).toHaveCount(2)
    await expect(setRevenueTerm.locator('.formula-entity-label').filter({ hasText: '옵션' })).toHaveCount(2)
    await expect(setRevenueTerm.locator('.formula-entity-role--set')).toHaveCSS('background-color', 'rgb(220, 252, 231)')
    await expect(setRevenueTerm.locator('.formula-entity-role--left')).toHaveCSS('background-color', 'rgb(199, 210, 254)')
    ...
    await expect(conversionRevenueTerm.locator('.formula-entity-card--left')).toContainText('P00000BV')
    await expect(conversionRevenueTerm.locator('.formula-entity-card--left')).toContainText('A ·')
    await expect(conversionRevenueTerm.locator('.formula-entity-label').filter({ hasText: '상품' })).toHaveCount(2)
    await expect(conversionRevenueTerm.locator('.formula-entity-label').filter({ hasText: '옵션' })).toHaveCount(2)
