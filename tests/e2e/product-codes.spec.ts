...
test.describe('상품코드 페이지', () => {
  ...
  test('교차셀 매핑 변경 더블클릭은 전환상품에서만 동작한다', async ({ page }) => {
    ...
    const targetCellSummary = page.locator('.lu-confirm-dialog').getByLabel('더블클릭한 셀')
    await expect(targetCellSummary).toContainText('U상품 P00000QE / G')
    await expect(targetCellSummary).toContainText('L상품 P00000BV / A')
