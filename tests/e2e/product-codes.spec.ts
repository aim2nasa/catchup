...
test.describe('상품코드 페이지', () => {
  ...
  test('교차셀 매핑 변경 더블클릭은 전환상품에서만 동작한다', async ({ page }) => {
    ...
    await expect(page.locator('.lu-confirm-cell-spotlight')).toBeVisible()
    const conversionSpotlightBox = await page.locator('.lu-confirm-cell-spotlight').boundingBox()
    expect(conversionSpotlightBox).toBeTruthy()
    expect(conversionSpotlightBox!.width).toBeGreaterThan(8)
    expect(conversionSpotlightBox!.height).toBeGreaterThan(8)
