...
test.describe('상품코드 페이지', () => {
  ...
  test('교차셀 더블클릭은 세트상품 기존 구성과 전환상품 매핑을 구분한다', async ({ page }) => {
    ...
    await expect(conversionCrossCell).toHaveClass(/lu-link-cell--after/)
    await expect(conversionCrossCell).toHaveClass(/pc-excel-col-selected/)
    await expect(page.locator('.lu-link-cell--before')).toHaveCount(1)
    await expect(page.locator('.lu-sheet-link-overlay')).toBeVisible()
    await expect(page.locator('.lu-sheet-link--before')).toHaveCount(1)
    await expect(page.locator('.lu-sheet-link--after')).toHaveCount(1)
    await expect(page.locator('.lu-confirm-dialog')).toContainText(/매핑셀|매핑 대상/)
    const tableWrap = page.locator('.pc-excel-table-wrap')
    const scrollBefore = await tableWrap.evaluate((element) => element.scrollTop)
    const tableBox = await tableWrap.boundingBox()
    expect(tableBox).toBeTruthy()
    await page.mouse.move(tableBox!.x + 24, tableBox!.y + tableBox!.height - 24)
    await page.mouse.wheel(0, 240)
    await expect.poll(() => tableWrap.evaluate((element) => element.scrollTop)).toBeGreaterThan(scrollBefore)
    await page.getByRole('button', { name: '취소' }).click()

    const changingTargetCell = page.locator('td[data-row-key="variant:P0000BIF:P0000BIF00CJ"][data-col-key="B:P00000QD-CN"]')
    await changingTargetCell.dblclick()
    const changingDialog = page.locator('.lu-confirm-dialog')
    await expect(changingDialog).toBeVisible()
    await expect(page.locator('.lu-link-cell--before')).toHaveCount(1)
    await expect(page.locator('.lu-link-cell--after')).toHaveCount(1)
    await expect(page.locator('.lu-sheet-link--before')).toHaveCount(1)
    await expect(page.locator('.lu-sheet-link--after')).toHaveCount(1)
    await expect(changingTargetCell).toHaveClass(/lu-link-cell--after/)
    await expect(changingTargetCell).toHaveClass(/pc-excel-col-selected/)
    await expect(changingDialog.getByRole('heading')).toHaveText('이 U상품의 매핑 대상인 L상품을 변경할까요?')
    await expect(changingDialog.getByLabel('매핑 개념')).toContainText('U상품')
    await expect(changingDialog.getByLabel('매핑 개념')).toContainText('전환상품')
    await expect(changingDialog.getByLabel('매핑 개념')).toContainText('L상품')
    await expect(changingDialog.getByLabel('매핑 개념')).toContainText('집계기준')
    await expect(changingDialog.getByLabel('매핑 개념')).toContainText('U상품이 어떠한 L상품과 관계되는지를 정의합니다.')
    await expect(changingDialog.locator('.lu-confirm-product-block--existing')).toContainText('상품코드 P00000BW')
    await expect(changingDialog.locator('.lu-confirm-product-block--existing')).toContainText('옵션 A')
    await expect(changingDialog.locator('.lu-confirm-product-block--existing')).toContainText('라이코플렉스 바닐라')
    await expect(changingDialog).toContainText('매핑 기준 U상품/옵션')
    await expect(changingDialog).toContainText('변경 전: 해제될 L상품/옵션')
    await expect(changingDialog).toContainText('변경 후: 지정할 L상품/옵션')
    await expect(changingDialog).toContainText('상품코드 P0000BIF')
    await expect(changingDialog).toContainText('옵션 CJ')
    await expect(changingDialog).toContainText('로제트 비즈왁스 800g')
    const existingTargetTitle = await changingDialog.locator('.lu-confirm-product-block--existing dd').getAttribute('title')
    expect(existingTargetTitle).toContain('P00000BW / A')
  ...
  test('전환상품 헤더 모달에서 옵션별 1:1 매핑을 확인하고 변경/취소/제외할 수 있다', async ({ page }) => {
    ...
    await expect(modal.locator('[data-conversion-row="P00000QE|G"] .conversion-editor-option-readonly')).toHaveText('A · -')
    await expect(modal.locator('select[aria-label="P00000QE-G L옵션"]')).toHaveCount(0)
    ...
    await page.mouse.move(dragHandleBox!.x + 80, dragHandleBox!.y + 24)
    await page.mouse.down()
    await page.mouse.move(dragHandleBox!.x + 170, dragHandleBox!.y - 32)
    await page.mouse.up()
    const movedBox = await modal.boundingBox()
    expect(movedBox).not.toBeNull()
    expect(movedBox!.x).toBeGreaterThan(initialBox!.x + 20)
    expect(movedBox!.y).toBeLessThan(initialBox!.y - 15)
    ...
    await page.mouse.down()
    await page.mouse.move(resizeHandleBox!.x - 120, resizeHandleBox!.y + 80)
    await page.mouse.up()
    ...
    await modal.getByLabel('P00000QE-G L상품', { exact: true }).selectOption('P00000ZB')
    await expect(modal.getByLabel('P00000QE-G L옵션', { exact: true })).toHaveValue('A')
    await modal.getByLabel('P00000QE-G L옵션', { exact: true }).selectOption('B')
    await expect(modal.locator('[data-conversion-row="P00000QE|G"] .conversion-editor-price').nth(1)).toHaveText('₩6,200')
  ...
  test('계산식 셀은 기본 원문 수식만 보이고 자세히/더블클릭 때 계산 내역을 펼친다', async ({ page }) => {
    ...
    const total = explanation.locator('.formula-explanation-total')
    ...
    await expect(directRevenueTerm.locator('.formula-term-number[title="수량"]')).toHaveText('75')
    await expect(directRevenueTerm.locator('.formula-term-amount')).toHaveText('₩1,972,500')
    const setRevenueTerm = terms.filter({ hasText: '세트 구성 매출' }).first()
    ...
    await expect(setRevenueTerm.locator('.formula-entity-role--set')).toHaveCSS('background-color', 'rgb(220, 252, 231)')
    await expect(setRevenueTerm.locator('.formula-entity-role--left')).toHaveCount(0)
    await expect(terms.locator('.formula-term-calculation')).toHaveCount(0)
    ...
    await expect(terms.nth(1).locator('.formula-term-index')).toHaveText('2')
    await expect(total).toContainText('직접판매 75')
    await expect(total).toContainText('위쪽상품 환산 2')
